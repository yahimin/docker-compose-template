...
class UserRegistrationSerializer(serializers.ModelSerializer):
    ...
    password_second = serializers.CharField(style={'input_type' : 'password_second'}, write_only=True)
    ...
    class Meta:
        ...
        r""" password 필드 쓰기만 가능하게 (사용자가 데이터를 전송할때는 필드에 값을 쓸수있지만 ,
        응답으로 데이터를 보낼때는 필드가 포함되지 않게)"""
        
        extra_kwargs={
            'password' : {'write_only': True},
            'password_second' : {'write_onlye' : True}
        }
        ...
        def validate(self,attrs):
            ...
            if len(password) > 8 or len(password_second) > 8:
                return serializers.ValidationError('User Password len is incorrect')

            if password.isdigit() or password_second.isdigit():
                return serializers.ValidationError('User Password is entirely numberic')
